...
import { useGetAggregatedSalesQuery } from '@slices/dashboardApiSlice';
...
const SalesTable = () => {
  ...
  const {
    ...
    error: gstSalesError,
  } = useGetAggregatedSalesQuery({ year });
          ...
          <div className='scrollbar-none overflow-x-scroll'>
            <table className='mt-4 min-w-full divide-y divide-gray-300'>
              ...
              <tbody className='divide-y divide-gray-200'>
                {sales?.monthly_data.map((sale, index) => (
                  <tr key={index}>
                ...
                <tr>
                  ...
                  <td className='px-3 py-4 text-sm font-semibold whitespace-nowrap text-gray-900 capitalize'>
                    ₹{formattedAmount(sales.total)}
                  </td>
                  <td
                    className={`px-3 py-4 text-sm font-semibold whitespace-nowrap capitalize ${sales.balanceDue ? 'text-red-500' : 'text-gray-900'}`}>
                    ₹{formattedAmount(sales.balanceDue)}
                  </td>
                  <td className='px-3 py-4 text-sm font-semibold whitespace-nowrap text-gray-900 capitalize'>
                    ₹{formattedAmount(sales.netTotal)}
                  </td>
                </tr>
              </tbody>
            </table>
