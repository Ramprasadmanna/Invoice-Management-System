...
import { useGetAggregatedGstSalesQuery } from '@slices/dashboardApiSlice';
...
const GstSalesTable = () => {
  ...
  const {
    data,
    isFetching: gstSalesLoading,
    isError: isGstSalesError,
    error: gstSalesError,
  } = useGetAggregatedGstSalesQuery({ year });
              ...
              <tbody className='divide-y divide-gray-200'>
                {data?.monthly_data.map((sale, index) => (
                  <tr key={index}>
                    ...
                    <td
                      className={`px-3 py-4 text-left text-sm whitespace-nowrap capitalize ${sale.balanceDue ? 'text-red-500' : 'text-gray-500'} `}>
                      ₹{formattedAmount(sale.balanceDue)}
                    </td>
                    <td className='px-3 text-left text-sm font-medium whitespace-nowrap text-gray-900'>
                      ₹{formattedAmount(sale.netTotal)}
                    </td>
                  </tr>
                ))}
                ...
                <tr>
                  ...
                  <td className='px-3 py-4 text-sm font-semibold whitespace-nowrap text-gray-900 capitalize'>
                    ₹{formattedAmount(data.taxableAmount)}
                  </td>
                  <td className='px-3 py-4 text-sm font-semibold whitespace-nowrap text-gray-900 capitalize'>
                  ...
                  <td
                    className={`px-3 py-4 text-sm font-semibold whitespace-nowrap capitalize ${data.balanceDue ? 'text-red-500' : 'text-gray-900'}`}>
                    ₹{formattedAmount(data.balanceDue)}
                  </td>
                  <td className='px-3 py-4 text-sm font-semibold whitespace-nowrap text-gray-900 capitalize'>
                    ₹{formattedAmount(data.netTotal)}
                  </td>
                </tr>
              </tbody>
            </table>
