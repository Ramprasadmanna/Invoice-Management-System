...
import ItemsForm from './GstPruchaseForm';
...
const GstPurchaseScreen = () => {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } =
    useGetGstPurchaseQuery({ keyword, pageNumber, pageSize, ...filter });

  // API Slice Mutations
  const [addGstPurchase, { isLoading: addGstPurchaseLoading }] =
    useAddGstPurchaseMutation();

  const [updateItem, { isLoading: updatePurchaseLoading }] =
    useUpdateGstPurchaseMutation();
  ...
  const handleSubmitData = async (itemData) => {
    if (initinalData) {
      ...
      try {
        await addGstPurchase(itemData).unwrap();
  ...
  return (
    <div className='scrollbar-none flex flex-col gap-6 overflow-x-hidden overflow-y-scroll p-4 sm:p-6'>
      ...
      <div className='flex flex-col justify-between gap-6 xl:flex-row'>
        <p className='text-2xl font-semibold md:text-3xl'>GST Purchase</p>
            ...
            {isFetching ? (
              ...
              <>
                <table className='min-w-full border-collapse border border-slate-500 sm:mb-6'>
                  ...
                  <tbody>
                    {data.gstPurchaseData.map((purchase, index) => (
                      <tr key={index}>
                        ...
                        <td className='border border-slate-400 px-3 py-4 text-sm whitespace-nowrap text-gray-900 capitalize'>
                          {purchase.item.companyName}
                        </td>
                        <td className='border border-slate-400 px-3 py-4 text-sm whitespace-nowrap text-gray-900'>
                          {purchase.item.state}
                        </td>
                        <td className='border border-slate-400 px-3 py-4 text-sm whitespace-nowrap text-gray-900'>
                          {purchase.item.gstNumber}
                        </td>
                        ...
                        <td className='gap-2 border border-slate-400 px-3 py-3.5 text-sm font-medium whitespace-nowrap text-gray-500'>
                          <div className='flex gap-4'>
                            ...
                            <Delete
                              queryfunc={useDeleteGstPurchaseMutation}
                              id={purchase.id}
                              message='GST Purchase Deleted Sucessfully'
                            />
                          </div>
                        </td>
                      </tr>
...
export default GstPurchaseScreen;
