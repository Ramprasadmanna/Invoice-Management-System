...
const ExpenseItemSummaryScreen = () => {
  ...
  // API Slice Query
  const { data, isError, error, isFetching } = useExpenseItemsSummaryQuery({
    itemId,
    year,
  });
  ...
  const handleDownloadExcel = async () => {
    try {
      const url = await downloadExcel({ year, itemId }).unwrap();
    ...
    <div className='scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-300 flex flex-col gap-8 overflow-x-hidden overflow-y-scroll p-4 sm:p-6'>
      ...
      <div className='flex flex-col items-center justify-between gap-6 xl:flex-row'>
        <p className='text-2xl font-semibold'>
          Expense Item Summary ({`${year}-${year + 1}`})
        </p>
      ...
      {isFetching ? (
        ...
        <div className='flex flex-col gap-4'>
          {data.map((purchaseDetails, index) => (
            <Summary purchaseDetails={purchaseDetails} key={index} />
          ))}
        </div>
      ) : (
        <Alert type='info'>No Expense Found</Alert>
      )}
    </div>
  );
};

export default ExpenseItemSummaryScreen;
