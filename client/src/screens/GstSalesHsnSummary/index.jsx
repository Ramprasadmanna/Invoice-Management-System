...
const GstSalesHsnSummaryScreen = () => {
  ...
  // API Slice Query
  const { data, isError, error, isFetching } = useGetGstSalesHsnSummaryQuery({
    hsnSac,
    year,
  });
  ...
  const handleDownloadExcel = async () => {
    try {
      const url = await downloadExcel({ hsnSac, year }).unwrap();
    ...
    <div className='scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-300 flex flex-col gap-8 overflow-x-hidden overflow-y-scroll p-4 sm:p-6'>
      ...
      <div className='flex flex-col items-center justify-between gap-6 xl:flex-row'>
        <p className='text-2xl font-semibold'>
          GST Sales HSN Summary ({`${year}-${year + 1}`})
        </p>
      ...
      {isFetching ? (
        ...
        <div className='flex flex-col gap-4'>
          {data.map((hsnDetails, index) => (
            <Summary hsnDetails={hsnDetails} key={index} />
          ))}
        </div>
      ) : (
...
export default GstSalesHsnSummaryScreen;
