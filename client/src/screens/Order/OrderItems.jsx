...
import { useLazySearchCashItemsQuery } from '@slices/cashItemsApiSlice';
...
const OrderItems = ({
  ...
  const calculateAndSetOrderDetails = (updatedValues) => {
    const price = updatedValues.items
    ...
    const baseTotal =
      Number(price) + Number(shippingCharges) - Number(discount);
  ...
  // Handling Price Change For Items
  ...
  const handlePaymentType = (e) => {
    ...
    setOrderDetails((prev) => ({
      ...prev,
      advanceAmount: isFullPayment ? 0 : +advanceAmount || 0,
      balanceDue: isFullPayment
        ? 0
        : (orderDetails.total - +advanceAmount).toFixed(2),
    }));
  ...
  // Debounce Product Search Function
  useEffect(() => {
                ...
                productList.map((item) => (
                  <li
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <p
                        className={`h-fit rounded-xs px-1 py-0.5 text-[8px] leading-[10px] font-medium uppercase ${item.type === 'goods' ? 'bg-green-200 text-green-600' : 'bg-blue-200 text-blue-600'} `}>
                        {item.type}
                      </p>
                    </div>
                ...
                {orderDetails.items.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className='min-w-[400px] border border-slate-400 px-3 py-2 text-gray-900 capitalize'>
                      <div className='flex items-center gap-2'>
                        <p className='mb-1 text-xs font-semibold whitespace-nowrap text-gray-800'>
                          {item.name}
                        </p>
                    ...
                    <td className='border border-slate-400 px-3 py-2 text-right text-xs whitespace-nowrap text-gray-900'>
                      ...
                      <input
                        ...
                        className='block w-full min-w-24 rounded-md border-transparent bg-slate-100 px-3 py-1.5 text-right text-sm font-medium placeholder:text-slate-400 hover:border-slate-400 focus:border-slate-400 focus:ring-0 disabled:cursor-not-allowed'
                      />
                    </td>
                    <td className='border border-slate-400 px-3 py-2 text-right text-xs whitespace-nowrap text-gray-900'>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td
                    ...
                    ₹{formattedAmount(orderDetails.price)}
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td
                    className={`${+orderDetails.totalDiscount > 0 ? 'text-red-500' : 'text-gray-900'} border border-slate-400 px-3 py-2 text-right text-base font-semibold whitespace-nowrap`}>
                ...
                <tr>
                  ...
                  <td
                    className={`${+orderDetails.otherAdjustments < 0 && 'text-red-500'} border border-slate-400 px-3 py-2 text-right text-base font-semibold whitespace-nowrap text-gray-900`}>
