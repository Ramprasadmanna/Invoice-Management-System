...
import ItemsForm from './ExpenseItemForm';
...
} from '@slices/expenseItemsApiSlice';

const ExpenseItemScreen = () => {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } =
    useGetExpenseItemsQuery({ keyword, pageNumber, pageSize });

  // API Slice Mutations
  const [addExpenseItem, { isLoading: addGstPurchaseItemLoading }] =
    useAddExpenseItemMutation();

  const [updateExpenseItem, { isLoading: updateGstPurchaseItemLoading }] =
    useUpdateExpenseItemMutation();
  ...
  const handleSubmitData = async (itemData) => {
    if (initinalData) {
      try {
        ...
        toast.success('Expense Item Updated Sucessfully.');
        setShowForm(false);
        setInitinalData(null);
        refetch();
      } catch (error) {
      ...
      try {
        await addExpenseItem(itemData).unwrap();
        toast.success('Expense Item Added Sucessfully');
        setShowForm(false);
        refetch();
      } catch (error) {
        if (!checkSessionExpired(error, dispatch)) {
  ...
  return (
    <div className='scrollbar-none flex flex-col gap-6 overflow-x-hidden overflow-y-scroll p-4 sm:p-6'>
      ...
      <div className='flex flex-col justify-between gap-6 xl:flex-row'>
        <p className='text-2xl font-semibold md:text-3xl'>Expense Items</p>
                      ...
                      <tr key={index}>
                        ...
                        <td className='gap-2 border border-slate-400 px-3 py-3.5 text-sm font-medium whitespace-nowrap text-gray-500'>
                          <div className='flex gap-4'>
                            ...
                            <Delete
                              queryfunc={useDeleteExpenseItemMutation}
                              id={item.id}
                              message='Expense Item Deleted Sucessfully'
                            />
                          </div>
                        </td>
                      </tr>
