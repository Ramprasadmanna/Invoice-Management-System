...
import { dateFormatter, inputDateFormatter } from '@utils/dateTimeFormatter';
...
const GstSalesForm = ({
  ...
  initinalData,
  setInitinalData,
  createGstSalesLoading,
  updateGstSaleLoading,
}) => {
  // Forms UseState
  ...
  const handleVerifyData = (e) => {
    ...
    if (invoiceNumber) {
      const invoiceRegex =
        /^A(100[1-9]|10[1-9][0-9]|1[1-9][0-9]{2,}|[2-9][0-9]{3,})$/;

      if (!invoiceRegex.test(invoiceNumber)) {
        return toast.error(
          "Invalid Invoice Number! It should start with 'A' and be greater than 1000."
        );
      }
    }
    ...
    const taxType =
      CURRENT_STATE === customer.placeOfSupply.toLowerCase()
        ? {
            cgst: Number(orderDetails.cgst),
            sgst: Number(orderDetails.sgst),
          }
        : { igst: Number(orderDetails.igst) };
    ...
    <div
      ...
      <div className='relative z-50 flex h-[600px] w-[400px] flex-col rounded-md bg-white shadow-md md:w-[700px] lg:w-[800px]'>
        <p className='bg-wovBlue rounded-t-md py-5 text-center text-xl font-semibold text-white'>
          {initinalData ? 'Edit Gst Sale' : 'Add Gst Sale'}
        </p>
        <div
        ...
        <form
            ...
            <div className='customer-note col-span-full'>
              <div className='flex items-center gap-2'>
                ...
                <label
                  ...
                  {initinalData
                    ? 'Send Mail After Updating Gst Sale'
                    : 'Send Mail After Creating Gst Sale'}
                </label>
              </div>
            </div>
          </div>
          ...
          <div className='flex items-center justify-end border-t border-slate-900/10'>
            <button
              ...
              disabled={updateGstSaleLoading || createGstSalesLoading}>
              {initinalData
                ? updateGstSaleLoading
                  ? 'Updating GST Sale ....'
                  : 'Update GST Sale'
...
GstSalesForm.propTypes = {
  showForm: PropTypes.bool,
  setShowForm: PropTypes.func,
  handleSubmitData: PropTypes.func,
  createGstSalesLoading: PropTypes.bool,
  updateGstSaleLoading: PropTypes.bool,
