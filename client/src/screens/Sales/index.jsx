...
const CashSalesScreen = () => {
  ...
  const {
    ...
    error,
    isFetching,
  } = useGetCashSalesQuery({ keyword, pageNumber, pageSize, ...filter });

  // API Slice Mutation
  const [createCashSales, { isLoading: createCashSalesLoading }] =
    useCreateCashSaleMutation();

  const [updateCashSales, { isLoading: updateCashSalesLoading }] =
    useUpdateCashSaleMutation();
  ...
  const handleSubmitData = async (sale) => {
    if (initinalData) {
      try {
        await updateCashSales({ ...sale, id: initinalData.id }).unwrap();
        toast.success('Sale Updated Sucessfully.');
        setShowForm(false);
        setInitinalData(null);
      } catch (error) {
      ...
      try {
        await createCashSales(sale).unwrap();
  ...
  return (
    <div className='scrollbar-none flex flex-col gap-6 overflow-x-hidden overflow-y-scroll p-4 sm:p-6'>
                        ...
                        <tr key={index}>
                          ...
                          <td className='gap-2 border border-slate-400 px-3 py-3.5 text-sm font-medium whitespace-nowrap text-gray-500'>
                            <div className='flex gap-4'>
                              ...
                              <Preview
                                queryfunc={useLazyPreviewCashSaleInvoiceQuery}
                                id={invoice.id}
                              />

                              <SendMail
                                queryfunc={useLazySendSaleInvoiceMailQuery}
                                id={invoice.id}
                              />

                              <Download
                                queryfunc={useLazyDownloadCashSaleInvoiceQuery}
                                id={invoice.id}
                                fileName={invoice.invoiceNumber}
                              />

                              <Delete
                                queryfunc={useDeleteCashSaleMutation}
                                id={invoice.id}
                                message='Sale Deleted Sucessfully'
                              />
                            </div>
                          </td>
                        </tr>
      ...
      <SalesForm
        ...
        createCashSalesLoading={createCashSalesLoading}
        updateCashSalesLoading={updateCashSalesLoading}
      />
    </div>
  );
};

export default CashSalesScreen;
